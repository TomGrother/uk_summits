...
const ROUTE_ENABLED_IDS = [2036, 1633]; // Ben Nevis, Snowdon
...
const MAX_ROUTES_TO_STORE = 4;

// Curated well-known trailheads for popular summits, used in preference to
// the (unreliable) Overpass auto-discovery below.
const CURATED_TRAILHEADS = {
  2036: [ // Ben Nevis
    { lat: 56.8094, lng: -5.0726 }, // Glen Nevis Visitor Centre - Mountain Track
  ],
  1633: [ // Snowdon
    { lat: 53.1190, lng: -4.1215 }, // Llanberis Path (Victoria Terrace, Llanberis)
    { lat: 53.0571, lng: -4.0148 }, // Pen-y-Pass - Pyg Track
    { lat: 53.0560, lng: -4.0123 }, // Pen-y-Pass - Miners' Track
    { lat: 53.0769, lng: -4.1206 }, // Snowdon Ranger car park
    { lat: 53.0496, lng: -4.0823 }, // Rhyd Ddu car park
  ],
};
...
async function backfillTrailheads() {
  ...
  for (const summit of summits) {
    let working = [];
    try {
      const candidates = CURATED_TRAILHEADS[summit.id] || await findTrailheadCandidates(summit);
      if (orsKey) {
        for (const c of candidates) {
